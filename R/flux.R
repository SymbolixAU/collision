...
#' Distance corrected flight flux from point count data
#'
#' The flight flux is defined as the number of flights per minute through a 
#' m^2 of vertical space centred on an observer.  We consider that 
#' every flight through the cylinder intersects a rectangular plane of width 
#' equal to the effective detection radius. This works because in the 
#' probability of collision calculations [prob_collision_static()] and 
#' [prob_collision_dynamic()] project the volume of the turbine onto the same 
#' plane. 
#' 
...
observed_flux_point <- function(
  ...
  # Flights through observer plan
  n_obs <- (mean_event_size * n_events) / (2 * edr * effort)
  sd_obs <- (sd_event_size * n_events) / (2 * edr * effort)
  
  
  # Convert to flights through turbine plan
  flux <- n_obs * rotor_diameter / (2 * edr * effort)
  # TODO this should be SE shouldn't it?
  flux_sd <- sd_obs * rotor_diameter / (2 * edr * effort)
  
  result <- list(mean = flux, 
                 sd = flux_sd
                 )
