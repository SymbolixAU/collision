...
The turbine is split into it's static and dynamic components as described in @Smales2013. The static component includes:
...
While the dynamic component is "the area swept by the leading edges of rotor blades during the time that a bird would take to pass through the rotor-swept zone" [@Smales2013] which is defined by the turbine hub and rotor parameters as well as the length and speed of the bird (`bird_length` and `bird_speed`).
...
# TEO: The first clause here feels incomplete (as in no subject in the sentence), which makes the second clause struggle. 
# Maybe "The probability of collision (of one bird with one turbine) will be calculated considering a bird with no avoidance behaviour using the `collision` approach, and that approach will be compared to the basic Band..."?
# Also, we seem to be making the same assumption as the Band model in that the proportion of flights at rotor swept height are assumed to be uniform.
# I think the Band model assumes uniform flight height within hight bands, and the `collision` approach is to use a taller flight height window (i.e. Band 
# looks at every 1 meter of flight height and `collision` looks at "below rotor swept" and "above rotor swept" as the height windows).  
...
# TEO: At the very end of this sentence, the reference to "stochLAB::get_avg_prob_collision()" is to a function and not a package, right?
...
# TEO: For prop_at_height and prop_below_height: to mimic how Band style
# models estimate the probability of collision (first by calculating
# the probability using area then incorportating flight height), 
# can we set prop_at_height and prop_below_height to assume a uniform
# flight height and take that aspect out of the computation so that 
# the comparison is more equal?
v90_single$hh - v90_single$blade_length
# 21 meters from the base to the bottom of the rotor swept area.

equal <- c(21, v90_single$rotor_diam) / (21 + v90_single$rotor_diam)
# 0.1865009 0.8134991

# TEO: I also think this would be a good place to highlight how
# `collision` is different from Band-style estimators in that
# we aren't doing two steps (calculate probability of collision
# in the uniform flight height case, then applying [i.e. integrating]
# the estimated flight heights). In `collision` the steps happen
# simulatenously. 
...
# TEO: Should we update this section? I don't think we have
# used edr in this vignette to this point, so maybe that was from
# an earlier version of prob_collision_static and/or 
# prob_collision_dynamic?

### Hang on - what's that `edr`?

Note we set our effective detection radius (`edr`) to half the rotor diameter. Normally this parameter is calculated via distance sampling [@Buckland2001]. As the effective detection increases, this probability of collision given interaction also decreases. 
...
# TEO: Should we use gliding birds? Both of these calculations
# below use flapping birds. Since the collision model relies more
# heavily on a bird's speed than its width, it seems that we would be
# minimizing the profile of the bird when it is at risk, which, I think,
# would be more like gliding birds than flapping birds? 
...
# TEO: Does the above comment still apply? 
