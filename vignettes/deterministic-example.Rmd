...
### Gather survey, observation and detection radius data
...
In this example we are desktop truncating the observations to the height of the turbine^[This is the simplest way to calculate the flight flux through the height of the turbine and is most suitable when the field observations include heights. If you have an independent source of flight heights more analysis may be required, but the priciple remains - we consider only those flights below the max rotor swept height. ]. So `prop_at_height` and `prop_below_height` should sum to 1. Desktop truncating is the most straightforward way to avoid artificially inflating or deflating the flux through the turbine window.
