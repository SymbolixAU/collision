...
# PY: I think a histogram is more interpretable for the average person but I'm not sure
...
* Step 1 - Run `flight_flux_point()`, `turbine_flux()` and `flux_per_year()` - calculate flux through turbine per year. This can also be done in one step using `turbine_flux_year()`.
...
This is done in two steps, first (if the observations were from point count surveys) we calculate the observed flight flux through vertical airspace using `flight_flux_point()`. Second we apply this to the "turbine plane" (a rectangular "doorway" defined by the rotor diameter and rotor swept height) using `turbine_flux()` to obtain the flights through the turbine plane per minute.

```{r}

# flights / m / min
obs_flux_min <- flight_flux_point(
